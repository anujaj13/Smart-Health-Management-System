...
class EmbeddingClient:
    """Embedding client using LangChain Hugging Face Embeddings."""
...
class VectorStore:
    ...
    def initialize_index(self):
        ...
        # Get dictionary of existing index names and their dimensions
        indexes_info = {index.name: index.dimension for index in self.pc.list_indexes()}

        if self.index_name in indexes_info:
            existing_dim = indexes_info[self.index_name]
            if existing_dim != self.dimension:
                logger.warning(
                    f"Index {self.index_name} exists with dimension {existing_dim}, "
                    f"but current embedding model has dimension {self.dimension}. Re-creating index..."
                )
                self.pc.delete_index(self.index_name)
                sleep(2)  # Wait for deletion to propagate
                # Remove from dict so the creation block runs
                indexes_info.pop(self.index_name)

        if self.index_name not in indexes_info:
        ...
        if isinstance(data, list):
            for item in data:
                ...
                if isinstance(item, dict):
                    content = item.get('content', '') or item.get('text', '') or item.get('additional_info', '') or item.get('info', '')
                    metadata = {k: v for k, v in item.items() if k not in ['content', 'text', 'additional_info', 'info']}
                else:
                    content = str(item)
                    metadata = {}
